package com.example.qqrobot.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.example.qqrobot.entity.Expression;
import com.example.qqrobot.service.ExpressionService;
import com.example.qqrobot.mapper.ExpressionMapper;
import org.springframework.stereotype.Service;

/**
* @author Administrator
* @description 针对表【expression】的数据库操作Service实现
* @createDate 2023-05-02 10:18:30
*/
@Service
public class ExpressionServiceImpl extends ServiceImpl<ExpressionMapper, Expression>
    implements ExpressionService{

}




